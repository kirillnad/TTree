...
function looksLikeMarkdownBlocks(text = '') {
...
function convertPlainTextToHtml(text = '') {
  ...
  return paragraphs
    .map((chunk) => `<div>${transformInlinePlaceholders(chunk).replace(/\n/g, '<br />')}</div>`)
    .join('');
}
...
function parseBlockDetailLines(lines = []) {
  ...
  lines.forEach((rawLine) => {
      ...
      if (BLOCK_SECTION_MAP[label]) {
        ...
        if (rest) {
          if (currentSection === 'comments') {
            buffers.comments.push(rest.replace(/^[-*+]\s*/, ''));
          } else if (currentSection === 'attributes') {
            buffers.attributes.push(rest);
          } else if (currentSection === 'title') {
            buffers.title = rest;
          } else {
            buffers.content = buffers.content ? `${buffers.content}\n${rest}` : rest;
          }
        }
        return;
      }
    ...
    if (currentSection === 'comments') {
      buffers.comments.push(line.replace(/^[-*+]\s*/, ''));
    } else if (currentSection === 'attributes') {
      buffers.attributes.push(line);
    } else if (currentSection === 'title') {
      buffers.title = buffers.title ? `${buffers.title}\n${line}` : line;
    } else {
      buffers.content = buffers.content ? `${buffers.content}\n${line}` : line;
    }
  });
...
function parseMarkdownBlocksInput(text = '') {
  ...
  lines.forEach((line) => {
    ...
    if (current?.block) {
      current.block.detailLines.push(line);
    }
  });
...
function buildBlockPayloadFromParsed(parsed) {
  ...
  return {
    ...
    children: (parsed.children || [])
      .map((child) => buildBlockPayloadFromParsed(child))
      .filter(Boolean),
  };
}
...
async function insertParsedMarkdownBlocks(parsedBlocks = []) {
