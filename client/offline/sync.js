import { cacheArticle, cacheArticlesIndex, getCachedArticlesIndex, getCachedArticlesSyncMeta, updateCachedDocJson } from './cache.js';
...
import { startMediaPrefetchLoop, pruneUnusedMedia } from './media.js';
...
export function startBackgroundFullPull() {
  if (fullPullStarted) return;
    ...
    for (const row of index || []) {
      try {
        ...
        if (serverUpdatedAt && local?.updatedAt === serverUpdatedAt && local?.hasDocJson) {
          ...
          try {
            const emb = await rawApiRequest(`/api/articles/${encodeURIComponent(row.id)}/embeddings?since=${encodeURIComponent(serverUpdatedAt)}`);
