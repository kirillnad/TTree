...
- В режиме outline стандартный `articleToolbar` (блочный тулбар) скрыт; на его месте показывается `outlineToolbar` с командами TipTap.
  Команды в `outlineToolbar` сгруппированы по выпадающим меню: “Текст”, “Списки”, “Таблица”. Undo/Redo остаются отдельными кнопками.
  Для таблиц используем узлы/команды TipTap Table (эквивалент `TableKit`: `table/tableRow/tableHeader/tableCell`) с `resizable: true` и командами: вставка таблицы 2×2, add/delete row/column (до/после), toggle header row/column, merge/split cell (если применимо).
