...
## Offline-first (always-on) на PGlite
...
Локальная база (PGlite + IndexedDB) — это **рабочее состояние** приложения. Сервер — “источник истины” и синк, но UI не должен зависеть от наличия сети.
...
- При старте: если PGlite/IndexedDB отсутствует/пустая/повреждена → показываем статус “Восстанавливаем локальные данные…” и выполняем **полную синхронизацию** с сервера (full pull).
...
### Сборка PGlite локально (без CDN)

- PGlite собирается в локальный ESM-бандл: `TTree/client/offline/pglite.bundle.js`.
- Пересборка: `cd TTree && npm run build:pglite`.
...
- Outbox коалесится: на статью должен существовать максимум один “актуальный” `save_doc_json` (заменяет предыдущий).
...
- ВАЖНО: запуск UI не должен блокироваться на инициализации offline-базы (PGlite). Offline инициализируется в фоне; при долгой инициализации показываем тост “Инициализируем offline-базу…”.
- Автосейв outline может держать черновик doc_json в `localStorage` (на случай, если сеть пропала в момент сохранения). Черновик **не должен** затирать актуальный контент с сервера:
  - применяем локальный черновик только если он новее `article.updatedAt` или если на сервере doc_json пустой;
  - если черновик старее — удаляем его как устаревший.

### Что считается локально, а что на сервере

- Локально (PGlite): `articles` (мета), `article_doc_json`, денорм `outline_sections` (для поиска), локальное хранение `section_embeddings` и ранжирование cosine на клиенте.
...
- `classic` (обычный) поиск выполняется локально по `outline_sections` (PGlite; `LIKE/ILIKE`).
...
- `semantic` (векторный) поиск ранжируется **на клиенте**:
  ...
  - embeddings секций синкаются в PGlite (`/api/articles/{id}/embeddings`);
...
Решение:
- В клиенте работает узкий Service Worker `TTree/client/uploads-sw.js`, который кэширует и обслуживает **только** запросы к `/uploads/**` (stale-while-revalidate).
