...
- В режиме outline стандартный `articleToolbar` (блочный тулбар) скрыт; на его месте показывается `outlineToolbar` с командами TipTap.
  В режиме просмотра `outlineToolbar` показывает только Undo/Redo. В режиме редактирования команды сгруппированы по выпадающим меню: “Текст”, “Списки”, “Таблица” (Undo/Redo остаются отдельными кнопками).
