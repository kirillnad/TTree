...
- `Ctrl+↑/↓`: переместить текущую секцию среди sibling’ов (вверх/вниз).
- `Ctrl+→/←`: indent/outdent секции (смена уровня вложенности в дереве) + нормализация дерева.
- `Shift+→/←`: collapse/expand текущей секции.

Примечания:
- Если текущая секция свернута — `Shift+→` разворачивает, `Shift+←` сворачивает (направления можно инвертировать, но должны быть консистентны).
...
- Горячие клавиши `Shift+→/←` должны менять `collapsed` у текущей секции.

## Перемещение и изменение вложенности

### Перемещение среди sibling’ов: `Ctrl+↑/↓`
...
2. Любая секция сворачивается/разворачивается мышью и `Shift+→/←`.
3. `Ctrl+↑/↓` переносит секцию среди sibling’ов без потери детей.
4. `Ctrl+→/←` меняет вложенность секции, дерево нормализуется предсказуемо.
